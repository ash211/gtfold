 ...
 * */
   ...
   for (i = 1; i <= len-b ; i++) {
	...
	calcVWM(i,j, INFINITY_, INFINITY_);  /* Calculates V and WM array for element (i,j)*/
      else calcWM(i,j);   /* Calculates WM array for element (i,j)*/
    }
  }
...
#pragma omp parallel for private (i,j) schedule(guided)
	  ...
	  calcVWM( i, j, VBI[i][j], INFINITY_ );   /* Calculates V and WM arrays*/
        ...
        else calcWM(i,j);  /* Calculates WM element at (i,j) */
      }
    }
...
#pragma omp parallel for private (i,j) schedule(guided)
      ...
      for (i=1; i <= len-b; i++) {
        ...
        else calcWM(i, j);      /* Calculation of WM at (i,j)*/
      }
    }
  }
...
/* Having ip = i+1 and jp=j-1 creates a stack which is considered separately in eS function. So here the max value of jp could be j-2*/
  ...
  g = jp-ip-1; 
  if ( g < 3 ) { VBI[i][j] = VBIij; return; }  /* if g is lesser than 3, then you don't extend this value. In this case the second base case of g-1=j-i-2c-4 will also not make a valid gap value. */
  ...
  for ( b = 1; b <= MIN ( i-1, len-j ); b++ ) {
   ...
   /* First side is c - with closing base pair (i-b,j+b)*/
    ...
    if ( E > E1 ) { E = E1; ip = ip1; jp = jp1; }
    if ( E > E2 ) { E = E2; ip = ip2; jp = jp2; } 
    if ( VBI[i-b][j+b] > E ) { VBI[i-b][j+b] = E; }
  }

  if ( g == 3 ) { VBI[i][j] = VBIij; return; } /* In this case the gap g-1=2, which should not be extended.*/ 
  ip = i+c+2;
  jp = j-c-1;
  E1 = eL( i, j, ip, jp ) + V[indx[ip]+jp];
  E2 = eL( i, j, i+c+1, j-c-2) + V[indx[i+c+1]+j-c-2];
  if ( VBIij > E1 ) VBIij = E1;		
  if ( VBIij > E2 )  VBIij = E2;

  if ( E2 < E1 ) { ip = i+c+1; jp = j-c-2; }
...
void calcWM(int i, int j ){
 ...
 /* WMidjd = dangling base on both ith and jth side.  WMidj = dangling base on ith side. WMijd = dangling base on jth side. WMij = no dangling base on both sides  */
  int WMidjd,  WMidj, WMijd, WMij, WMijp;   int rnai, rnaj;
...
/* Take the minimum of all of the terms */
  ...
  WMij = MIN ( MIN( WM[i+1][j]+c, WM[i][j-1]+c ), WMij );
  WMij = MIN ( WMijp, WMij);
...
void calcVWM(int i, int j, int VBIij, int VMij) {
  ...
  /* WM starts */
  a = multConst[0]; b = multConst[2]; c = multConst[1];
  WMijp = INFINITY_;
...
void calcVMVWM(int i, int j) {
  ...
  int WMip1hm1 /* WM value at i+1 and j-1 */, WMip2hm1/* WM value at i+2 and h-1*/, WMhjm1/* WM value at h and j-1*/, WMhjm2/* WM value at h and j-2*/, WMhp1j /*WM value at h+1 and j*/;
  int rnai, rnaj;
  int tmp1, tmp2;
 ...
 * */
  for (h=i+6; h<=j-5; h++) {
    ...
    if ( ( A_temp <= VMij )  ) VMij = A_temp;
		
    A_temp = WMip2hm1 + WMhjm1;
    if ( A_temp <= VMidj ) VMidj = A_temp;
			
    A_temp = WMip1hm1 + WMhjm2;
    if ( A_temp <= VMijd  ) VMijd = A_temp;
		
    A_temp = WMip2hm1 + WMhjm2;
    if ( A_temp <= VMidjd )	VMidjd = A_temp;
  }
  ...
  V[indx[i]+j] = MIN(MIN(eH(i,j),es), MIN(VBI[i][j],VMij));
#if DEBUG
  if (indx[i]+j > (LENGTH-1)*(LENGTH)/2)
...
void calcVBIVMVWM(int i, int j) {
  ...
  int ip, jp, temp, VBIij, thres1;
  int a=multConst[0] /*a is an offset penalty for multiloops */, b=multConst[2]/* b is penalty for multiloop branches, one per branch*/, c=multConst[1] /*Penalty for single stranded nucleotides in the multiloops*/, a1, h, es;
  int aupen;
  int WMijp, WMidjd,  WMidj, WMijd, WMij;
  int VMij, VMijd, VMidj, VMidjd, A_temp;
  int WMip1hm1 /* WM value at i+1 and h-1 */, WMip2hm1 /* WM value at i+2 and j-1*/, WMhjm1 /*WM value at h and j-1*/, WMhjm2 /* WM value at h and j-2*/, WMhp1j /*WM value at h+1 and j*/;   
  int rnai, rnaj;
  int tmp1, tmp2;
 ...
 * */
  ...
  V[indx[i]+j] = MIN(MIN(eH(i,j),es), MIN(VBIij,VMij));
#if DEBUG
  if (indx[i]+j > (LENGTH-1)*(LENGTH)/2)
    fprintf(stderr,"ERROR: in calcVBIVMVWM: i: %5d  j: %5d\n",i,j);
#endif
...
void calcW(int j) {
  int i; 
  int Wj, Widjd /*Dangling base on both sides*/, Wijd/* Dangling base on jth side.*/, Widj/* Dangling base on ith side */, Wij/* No dangle base on any of the sides */, Wim1 /* Value of W at (i-1). Set to zero if positive*/;
  int rnai, rnaj;
...
int eL(int i, int j, int ip, int jp) {
  ...
  if (size1 == 0 || size2 == 0 ) {
	...
	energy = tstki[fourBaseIndex(RNA[i],RNA[j],RNA[i+1],RNA[j-1])]
	  + tstki[fourBaseIndex(RNA[jp],RNA[ip],RNA[jp+1],RNA[ip-1])]
	  + inter[30]
	  + loginc
	  + eparam[3]
	  + MIN( maxpen, (lopsided * poppen[MIN(2, MIN(size1, size2))]));
      }
	...
	+ inter[size]
	+ loginc
	+ eparam[3]  /* AM: I don't understand this eparam value, I think they do not play any role currently. Please look in loader.cc file, for what value have been assinged to various elements of eparam array */
	+ MIN( maxpen, (lopsided * poppen[MIN(2, MIN(size1, size2))]));   /*  */
    }
  }
...
int eH( int i, int j) {
  ...
  if (size > 30){   
    ...
    energy =  hairpin[30] + loginc + tstkh[fourBaseIndex(RNA[i],RNA[j],RNA[i+1],RNA[j-1])] + eparam[4]; /* size penalty + terminal mismatch stacking energy*/
  }
  
  else if (size <= 30 && size > 4) {
    energy = hairpin[size] + tstkh[fourBaseIndex(RNA[i],RNA[j],RNA[i+1],RNA[j-1])] + eparam[4]; /* size penalty + terminal mismatch stacking energy*/
  } 
    ...
    for (index = 0; index < 6; ++index) {
      switch (RNA[i+index]) {
      case BASE_A: kmult = 1; break;
      case BASE_C: kmult = 2; break;
      case BASE_G: kmult = 3; break;
      case BASE_U: kmult = 4; break;
      default: kmult = 0; fprintf(stderr,"ERROR: in tetraloop calculation\n");
      }
    ...
    energy = tlink + hairpin[size] + tstkh[fourBaseIndex(RNA[i],RNA[j],RNA[i+1],RNA[j-1])] + eparam[4];
  } 
    ...
    if ((RNA[i] == BASE_A &&  RNA[j] == BASE_U) || (RNA[i] == BASE_U &&  RNA[j] == BASE_A)) {
  ...
  else if (size == 0) return INFINITY_;
  
  
  /*  GGG Bonus => GU closure preceded by GG */
  /*  i-2 = i-1 = i = G, and j = U; i < j */
  if (i > 2) {
    if (RNA[i-2]== BASE_G && RNA[i-1]== BASE_G && RNA[i]== BASE_G && RNA[j]==BASE_U) {
      energy += gubonus;			
