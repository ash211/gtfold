...
void populate() {
  ...
  cout << "Loading in GTfold data files from " << xstr(DATADIR) << endl;
...
// Initialize the array with INFINITY
  ...
  fileName = xstr(DATADIR) "/" + fileName;
...
int initLoopValues(string fileName) { 
  ...
  while (index < 30) {		
	...
	if (strcmp(currentWord, ".")) {
      ...
      switch (j) {
      case 2 : inter[index] = tempValue; break;
      case 3 : bulge[index] = tempValue; break;
      case 4 : hairpin[index] = tempValue; break;
      }
    }
  }
  cf.close();
